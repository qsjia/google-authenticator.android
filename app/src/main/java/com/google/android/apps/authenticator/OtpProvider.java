...
public class OtpProvider implements OtpSource {
  ...
  private String getCurrentCode(String username, byte[] challenge) throws OtpSourceException {
    ...
    Integer digits = mAccountDb.getDigits(username);
    ...
    return computePin(secret, otp_state, challenge, digits);
  }
   ...
   * @return the PIN
   */
  private String computePin(String secret, long otp_state, byte[] challenge, Integer digits)
    ...
    try {
      ...
      PasscodeGenerator pcg = new PasscodeGenerator(signer,
        (challenge == null) ? (digits == 0 ? PIN_LENGTH : digits) : REFLECTIVE_PIN_LENGTH);
