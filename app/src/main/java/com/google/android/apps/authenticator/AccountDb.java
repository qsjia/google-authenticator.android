...
public class AccountDb {
  ...
  public static final Integer DEFAULT_DIGITS = 6;
  ...
  public AccountDb(Context context) {
    ...
    mDatabase.execSQL(String.format(
        ...
        DEFAULT_HOTP_COUNTER, TYPE_COLUMN, DIGITS_COLUMN,
  ...
  public Integer getDigits(String email) {
      Cursor cursor = getAccount(email);
      try {
          if (!cursorIsEmpty(cursor)) {
              cursor.moveToFirst();
              return cursor.getInt(cursor.getColumnIndex(DIGITS_COLUMN));
          }
      } finally {
          tryCloseCursor(cursor);
      }
  ...
  void setDigits(String email, Integer digits) {
      ContentValues values = new ContentValues();
      values.put(EMAIL_COLUMN, email);
      values.put(DIGITS_COLUMN, digits);
      mDatabase.update(TABLE_NAME, values, whereClause(email), null);
  }
  ...
  public void update(String email, String secret, String oldEmail,
      OtpType type, Integer digits, Integer counter) {
    update(email, secret, oldEmail, type, digits, counter, null);
  }
   ...
   */
  public void update(String email, String secret, String oldEmail,
      OtpType type, Integer digits, Integer counter, Boolean googleAccount) {
    ContentValues values = new ContentValues();
    values.put(EMAIL_COLUMN, email);
