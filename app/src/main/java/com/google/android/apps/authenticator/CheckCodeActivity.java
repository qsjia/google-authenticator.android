...
public class CheckCodeActivity extends Activity {
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    Integer digits = accountDb.getDigits(user);
    String checkCode = null;
    String errorMessage = null;
    try {
      checkCode = getCheckCode(secret, digits);
  ...
  static String getCheckCode(String secret, Integer digits) throws GeneralSecurityException,
      DecodingException {
    final byte[] keyBytes = Base32String.decode(secret);
    Mac mac = Mac.getInstance("HMACSHA1");
    mac.init(new SecretKeySpec(keyBytes, ""));
    PasscodeGenerator pcg = new PasscodeGenerator(mac, digits);
